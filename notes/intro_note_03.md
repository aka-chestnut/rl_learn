...
![agent_env](intro_note_03/agent_env.png)
...
一系列的agent和environment**交互序列**。每个episode之间相互不影响，且都是有一个相同的**终止状态**（**terminate state**）。
...
在Episode的尾部加入 **吸收状态**（**absorbing state**），在此状态下，奖励永远是0，且下一个状态永远是当前状态。
...
- 用 $p(s',r|s,a)$ 表示 $Pr\{S_{t+1}={s}', R_{t+1}=r|S_t,A_t\}$ ，这个条件概率是描绘了整个MDP的**动态**（**Dynamics**）。
...
- state-action-next state期望奖励： $r(s, a, s') = \mathbb{E}[R_{t+1}|S_t=s,A_t=a, S_{t+1}=s'] = \sum_{r\in R}r\frac{p(s',r|s,a)}{p(s'|s,a)}$
...
- 关于策略 $\pi$ 的action-value函数：$q_{\pi}(a,s) = \mathbb{E}_{\pi}[G_t|S_t=s,A_t=a] = \mathbb{E}_{\pi}[\sum_{k=0}^{\infty}\gamma^k R_{t+k+1}|S_t=s,A_t=a]$
...
- 理解：
  ...
  - 2.Bellman Equation的本质是**表达某个状态的价值和其后继状态的价值之间的关系。**
...
![backup](intro_note_03/backup.png)
...
![backup_opt](intro_note_03/backup_opt.png)
...
![3_grid_world](intro_note_03/3_grid_world.jpg)
